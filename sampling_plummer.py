...
# --- Calculate best softening (d/10 rule) ---
R_hm = b / math.sqrt(2**(2/3) - 1.0)  # ~1.305 b
V_hm = (4.0 / 3.0) * math.pi * R_hm**3
n_density = (N / 2.0) / V_hm
d_mean = n_density**(-1.0 / 3.0)
eps_best = d_mean / 10.0

print(f"--- Plummer Sphere Setup ---")
print(f"Half-mass radius: {R_hm:.3f}")
print(f"Mean separation:  {d_mean:.3f}")
print(f"Best softening:   {eps_best:.4f} (using this in run.sh)")
...
def get_q():
    ...
    g_max = 0.092  # Max value of q^2 * (1 - q^2)^(7/2)
    while True:
        q = random.random()
        g = (q**2) * ((1.0 - q**2)**3.5)
